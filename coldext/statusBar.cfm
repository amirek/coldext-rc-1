...
<cfparam name="attributes.busyIconCls" default=""><!--- @@type: String @@help: The default iconCls applied when calling showBusy (defaults to 'x-status-busy'). It can be overridden at any time by passing the iconCls argument into showBusy. See the iconCls docs for additional details about customizing the icon. --->
<cfparam name="attributes.busyIconClsJS" default=""><!--- @@type: JS @@help: The default iconCls applied when calling showBusy (defaults to 'x-status-busy'). It can be overridden at any time by passing the iconCls argument into showBusy. See the iconCls docs for additional details about customizing the icon. --->
<cfparam name="attributes.busyText" default=""><!--- @@type: String @@help: The default text applied when calling showBusy (defaults to 'Loading...'). It can be overridden at any time by passing the text argument into showBusy. --->
<cfparam name="attributes.busyTextJS" default=""><!--- @@type: JS @@help: The default text applied when calling showBusy (defaults to 'Loading...'). It can be overridden at any time by passing the text argument into showBusy. --->
...
<cfparam name="attributes.cls" default=""><!--- @@type: String @@help: The base class applied to the containing element for this component on render (defaults to 'x-statusbar') --->
<cfparam name="attributes.clsJS" default=""><!--- @@type: JS @@help: The base class applied to the containing element for this component on render (defaults to 'x-statusbar') --->
...
<cfparam name="attributes.defaultIconCls" default=""><!--- @@type: String @@help: The default iconCls value (see the iconCls docs for additional details about customizing the icon). This will be used anytime the status bar is cleared with the useDefaults:true option (defaults to ''). --->
<cfparam name="attributes.defaultIconClsJS" default=""><!--- @@type: JS @@help: The default iconCls value (see the iconCls docs for additional details about customizing the icon). This will be used anytime the status bar is cleared with the useDefaults:true option (defaults to ''). --->
<cfparam name="attributes.defaultText" default=""><!--- @@type: String @@help: The default text value. This will be used anytime the status bar is cleared with the useDefaults:true option (defaults to ''). --->
<cfparam name="attributes.defaultTextJS" default=""><!--- @@type: JS @@help: The default text value. This will be used anytime the status bar is cleared with the useDefaults:true option (defaults to ''). --->
...
<cfparam name="attributes.iconCls" default=""><!--- @@type: String @@help: A CSS class that will be applied to the status element and is expected to provide a background image that will serve as the status bar icon (defaults to ''). The class is applied directly to the div that also contains the status text, so the rule should provide the appropriate padding on the div to make room for the image. Example usage:// Example CSS rule:.x-statusbar .x-status-custom {    padding-left: 25px;    background: transparent url(images/custom-icon.gif) no-repeat 3px 3px;}// Initializing the status bar:var sb = new Ext.StatusBar({    defaultIconCls: 'x-status-custom'});// Setting it in code:sb.setStatus({    text: 'New status',    iconCls: 'x-status-custom'}); --->
<cfparam name="attributes.iconClsJS" default=""><!--- @@type: JS @@help: A CSS class that will be applied to the status element and is expected to provide a background image that will serve as the status bar icon (defaults to ''). The class is applied directly to the div that also contains the status text, so the rule should provide the appropriate padding on the div to make room for the image. Example usage:// Example CSS rule:.x-statusbar .x-status-custom {    padding-left: 25px;    background: transparent url(images/custom-icon.gif) no-repeat 3px 3px;}// Initializing the status bar:var sb = new Ext.StatusBar({    defaultIconCls: 'x-status-custom'});// Setting it in code:sb.setStatus({    text: 'New status',    iconCls: 'x-status-custom'}); --->
...
<cfparam name="attributes.statusAlign" default=""><!--- @@type: String @@help: The alignment of the status element within the overall StatusBar layout. When the StatusBar is rendered, it creates an internal div containing the status text and icon. Any additional Toolbar items added in the StatusBar's items config, or added via add or any of the supported add* methods, will be rendered, in added order, to the opposite side. The status element is greedy, so it will automatically expand to take up all sapce left over by any other items. Example usage: // Create a left-aligned status bar containing a button,// separator and text item that will be right-aligned (default):new Ext.Panel({    title: 'StatusBar',    // etc.    bbar: new Ext.StatusBar({        defaultText: 'Default status text',        id: 'status-id',        items: [{            text: 'A Button'        }, '-', 'Plain Text']    })});// By adding the statusAlign config, this will create the// exact same toolbar, except the status and toolbar item// layout will be reversed from the previous example:new Ext.Panel({    title: 'StatusBar',    // etc.    bbar: new Ext.StatusBar({        defaultText: 'Default status text',        id: 'status-id',        statusAlign: 'right',        items: [{            text: 'A Button'        }, '-', 'Plain Text']    })}); --->
<cfparam name="attributes.statusAlignJS" default=""><!--- @@type: JS @@help: The alignment of the status element within the overall StatusBar layout. When the StatusBar is rendered, it creates an internal div containing the status text and icon. Any additional Toolbar items added in the StatusBar's items config, or added via add or any of the supported add* methods, will be rendered, in added order, to the opposite side. The status element is greedy, so it will automatically expand to take up all sapce left over by any other items. Example usage: // Create a left-aligned status bar containing a button,// separator and text item that will be right-aligned (default):new Ext.Panel({    title: 'StatusBar',    // etc.    bbar: new Ext.StatusBar({        defaultText: 'Default status text',        id: 'status-id',        items: [{            text: 'A Button'        }, '-', 'Plain Text']    })});// By adding the statusAlign config, this will create the// exact same toolbar, except the status and toolbar item// layout will be reversed from the previous example:new Ext.Panel({    title: 'StatusBar',    // etc.    bbar: new Ext.StatusBar({        defaultText: 'Default status text',        id: 'status-id',        statusAlign: 'right',        items: [{            text: 'A Button'        }, '-', 'Plain Text']    })}); --->
...
<cfparam name="attributes.text" default=""><!--- @@type: String @@help: A string that will be rendered into the status element as the status message (defaults to ''); --->
<cfparam name="attributes.textJS" default=""><!--- @@type: JS @@help: A string that will be rendered into the status element as the status message (defaults to ''); --->
...
<cfoutput>
	<cfif AttributeNotEmpty("var")>var #attributes.var# = </cfif>new Ext.StatusBar({
		...
		<cfif AttributeNotEmpty("autoClear")>autoClear: #attributes.autoClear#,</cfif>
		...
		<cfif AttributeNotEmpty("busyIconCls")>busyIconCls: "#JSStringFormat(attributes.busyIconCls)#",</cfif>
		<cfif AttributeNotEmpty("busyIconClsJS")>busyIconCls: #attributes.busyIconClsJS#,</cfif>
		<cfif AttributeNotEmpty("busyText")>busyText: "#JSStringFormat(attributes.busyText)#",</cfif>
		<cfif AttributeNotEmpty("busyTextJS")>busyText: #attributes.busyTextJS#,</cfif>
		...
		<cfif AttributeNotEmpty("defaultIconCls")>defaultIconCls: "#JSStringFormat(attributes.defaultIconCls)#",</cfif>
		<cfif AttributeNotEmpty("defaultIconClsJS")>defaultIconCls: #attributes.defaultIconClsJS#,</cfif>
		<cfif AttributeNotEmpty("defaultText")>defaultText: "#JSStringFormat(attributes.defaultText)#",</cfif>
		<cfif AttributeNotEmpty("defaultTextJS")>defaultText: #attributes.defaultTextJS#,</cfif>
		...
		<cfif AttributeNotEmpty("statusAlign")>statusAlign: "#JSStringFormat(attributes.statusAlign)#",</cfif>
		<cfif AttributeNotEmpty("statusAlignJS")>statusAlign: #attributes.statusAlignJS#,</cfif>
