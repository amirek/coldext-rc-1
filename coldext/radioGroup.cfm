...
<!--- defaults --->
<cfparam name="attributes.name" default="radioGroup#Int(Rand()*1000)#">
...
<!--- attributes --->
<cfparam name="attributes.allowBlank" default=""><!--- @@type: Boolean @@help: True to allow every item in the group to be blank (defaults to false). If allowBlank = false and no items are selected at validation time, {@link @blankText} will be used as the error text. --->
...
<cfparam name="attributes.blankText" default=""><!--- @@type: String @@help: Error text to display if the allowBlank validation fails (defaults to "You must select one item in this group") --->
<cfparam name="attributes.blankTextJS" default=""><!--- @@type: JS @@help: Error text to display if the allowBlank validation fails (defaults to "You must select one item in this group") --->
...
<!--- check columns --->
<cfif isNumeric(attributes.columns)>
	<cfset attributes.columnsJS = Int(attributes.columns)>
...
<cfoutput>
	<cfif AttributeNotEmpty("var")>var #attributes.var# = </cfif>new Ext.form.RadioGroup({
