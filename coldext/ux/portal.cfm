...
<!--- ux resources --->
<cfinclude template="resources/portal.cfm">


<!--- defaults --->
<cfparam name="attributes.xtype" default="portal">
...
<cfparam name="attributes.autoHeight" default=""><!--- @@type: Boolean @@help: True to use height:'auto', false to use fixed height (defaults to false). Note: Setting autoHeight:true means that the browser will manage the panel's height based on its contents, and that Ext will not manage it at all. If the panel is within a layout that manages dimensions (fit, border, etc.) then setting autoHeight:true can cause issues with scrolling and will not generally work as expected since the panel will take on the height of its contents rather than the height required by the Ext layout. --->
...
<cfparam name="attributes.disabled" default=""><!--- @@type: Boolean @@help: Render this panel disabled (default is false). An important note when using the disabled config on panels is that IE will often fail to initialize the disabled mask element correectly if the panel's layout has not yet completed by the time the Panel is disabled during the render process. If you experience this issue, you may need to instead use the afterlayout event to initialize the disabled state: new Ext.Panel({    ...    listeners: {        'afterlayout': {            fn: function(p){                p.disable();            },            single: true // important, as many layouts can occur        }    }}); --->
...
<cfparam name="attributes.region" default=""><!--- @@type: String @@help: The position within the viewport to attach this component to. The 'center' region is always required when using a viewport. @@options: center,east,north,south,west --->
<cfparam name="attributes.regionJS" default=""><!--- @@type: JS @@help: The position within the viewport to attach this component to. The 'center' region is always required when using a viewport. @@options: center,east,north,south,west --->
...
<cfif not thisTag.hasEndTag>
	<cfthrow message="portal missing closing tag">
</cfif>
...
<cfoutput>
	<cfif AttributeNotEmpty("var")>var #attributes.var# = new Ext.Panel(</cfif>{
		...
		<cfif AttributeNotEmpty("regionJS")>region: #attributes.regionJS#,</cfif>
