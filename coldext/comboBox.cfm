...
<!--- defaults --->
<cfparam name="attributes.name" default="comboBox#Int(Rand()*1000)#">
...
<cfparam name="attributes.triggerAction" default="all">
...
<cfparam name="attributes.autoCreate" default=""><!--- @@type: Boolean/Object @@help: A DomHelper element spec, or true for a default element spec (defaults to: {tag: "input", type: "text", size: "24", autocomplete: "off"}) --->
...
<cfparam name="attributes.selectOnFocus" default=""><!--- @@type: Boolean @@help: True to select any existing text in the field immediately on focus. Only applies when editable = true (defaults to false) --->
...
<cfparam name="attributes.triggerClass" default=""><!--- @@type: String @@help: An additional CSS class used to style the trigger button. The trigger will always get the class 'x-form-trigger' and triggerClass will be appended if specified (defaults to 'x-form-arrow-trigger' which displays a downward arrow icon). --->
<cfparam name="attributes.triggerClassJS" default=""><!--- @@type: JS @@help: An additional CSS class used to style the trigger button. The trigger will always get the class 'x-form-trigger' and triggerClass will be appended if specified (defaults to 'x-form-arrow-trigger' which displays a downward arrow icon). --->
...
<cfparam name="attributes.return" default="text"><!--- @@type: String @@options: value,text @@help: (ColdExt attribute) The data that is submitted by this ComboBox component. 'value' to use the selected item's underlying id/value, or 'text' to use the text that is shown in the control when an item is selected. --->
...
<cfset thisTag.options = ArrayNew(1)>
...
<cfoutput>
	<cfif AttributeNotEmpty("var")>var #attributes.var# = </cfif>new Ext.form.ComboBox({
		...
		<cfif AttributeNotEmpty("autoCreate")>autoCreate: #attributes.autoCreate#,</cfif>
		...
		<cfif arrayLen(thisTag.options)>
			<cfif attributes.triggerAction eq "">triggerAction: "all",</cfif>
			mode: "local",
			displayField: "text",
			valueField: "value",
			store: new Ext.data.SimpleStore({
				fields: ["value", "text"],
				data: [ <cfif arrayLen(thisTag.options)>#ArrayKeyToList(thisTag.options, "_out")#</cfif> ]
			}),
		<cfelse>
			<cfif AttributeNotEmpty("store")>store: #attributes.store#,</cfif>
		</cfif>

		<cfif attributes.return eq "value">
			hiddenName: "#JSStringFormat(attributes.name)#",
		<cfelse>
			name: "#JSStringFormat(attributes.name)#",
		</cfif>
		<!--- TODO: should name and hidden name be commented out in attributes body? --->
