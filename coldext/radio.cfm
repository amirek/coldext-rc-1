...
<!--- attribute state --->
<cfset _emptyName = AttributeEmpty("name")>

<!--- defaults --->
<cfparam name="attributes.name" default="radio#Int(Rand()*1000)#">
...
<cfparam name="attributes.inputValue" default="#attributes.value#">
...
<!--- radioGroup --->
<cfif parentTag eq "CF_RADIOGROUP">
	<cfif AttributeEmpty("boxLabel")>
		<cfset attributes.boxLabel = attributes.fieldLabel>
	</cfif>
	<cfif _emptyName>
		<!--- get name from radioGroup --->
		<cftry>
			<cfset radioGroupData = GetBaseTagData("CF_RADIOGROUP")>
			<cfset attributes.name = radioGroupData.attributes.name>
...
<cfoutput>
	<cfif AttributeNotEmpty("var")>var #attributes.var# = </cfif>new Ext.form.Radio({
