...
<cfparam name="attributes.autoEncode" default=""><!--- @@type: Boolean @@help: True to automatically HTML encode and decode values pre and post edit (defaults to false) --->
...
<cfparam name="attributes.clicksToEdit" default=""><!--- @@type: Number @@help: The number of clicks on a cell required to display the cell's editor (defaults to 2). Setting this option to 'auto' means that mousedown on the selected cell starts editing that cell. --->
...
<cfparam name="attributes.customEditors" default=""><!--- @@type: Object @@help: An object containing name/value pairs of custom editor type definitions that allow the grid to support additional types of editable fields. By default, the grid supports strongly-typed editing of strings, dates, numbers and booleans using built-in form editors, but any custom type can be supported and associated with a custom input control by specifying a custom editor. The name of the editor type should correspond with the name of the property that will use the editor. Example usage: var grid = new Ext.grid.PropertyGrid({    ...    customEditors: {        'Start Time': new Ext.grid.GridEditor(new Ext.form.TimeField({selectOnFocus:true}))    },    source: {        'Start Time': '10:00 AM'    }}); --->
...
<cfparam name="attributes.propertyNames" default=""><!--- @@type: Object @@help: An object containing property name/display name pairs. If specified, the display name will be shown in the name column instead of the property name. --->
...
<cfparam name="attributes.source" default=""><!--- @@type: Object @@help: A data object to use as the data source of the grid (see setSource for details). --->
...
<cfset thisTag.source = ArrayNew(1)>
...
<cfif not thisTag.hasEndTag>
	<cfthrow message="propertyGrid missing closing tag">
</cfif>
...
<cfoutput>
	<cfif AttributeNotEmpty("var")>var #attributes.var# = </cfif>new Ext.grid.PropertyGrid({
		...
		<cfif AttributeNotEmpty("autoEncode")>autoEncode: #attributes.autoEncode#,</cfif>
		...
		<cfif AttributeNotEmpty("clicksToEdit")>clicksToEdit: #attributes.clicksToEdit#,</cfif>
		...
		<cfif AttributeNotEmpty("customEditors")>customEditors: #attributes.customEditors#,</cfif>
		...
		<cfif AttributeNotEmpty("propertyNames")>propertyNames: #attributes.propertyNames#,</cfif>
		...
		<cfif AttributeNotEmpty("source")>source: #attributes.source#,</cfif>
		...
		<cfif arrayLen(thisTag.source)>source: { #ArrayKeyToList(thisTag.source, "_out")# },</cfif>
